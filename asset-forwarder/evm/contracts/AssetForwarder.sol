    ...
    MultiCaller
{
    ...
    modifier whenRelayNotPaused() {
        require(!relayPause, "Relay: paused");
        _;
    }
    ...
    function pause(
        bool _depositPause,
        bool _relayPause
    ) external onlyRole(PAUSER) {
        _pause(_depositPause, _relayPause);
    }

    function _pause(bool _depositPause, bool _relayPause) internal {
        if (_depositPause) {
            require(!depositPause, "Deposits are already paused");
            depositPause = true;
            emit Paused(msg.sender, 0);
        }
        if (_relayPause) {
            require(!relayPause, "Deposits are already paused");
            relayPause = true;
            emit Paused(msg.sender, 1);
        }
    }

    function unpause(
        bool _depositUnpause,
        bool _relayUnpause
    ) external onlyRole(PAUSER) {
        _unpause(_depositUnpause, _relayUnpause);
    }

    function _unpause(bool _depositUnpause, bool _relayUnpause) internal {
        if (_depositUnpause) {
            require(depositPause, "Deposits are already unpaused");
            depositPause = false;
            emit Unpaused(msg.sender, 0);
        }
        if (_relayUnpause) {
            require(relayPause, "Deposits are already unpaused");
            relayPause = false;
            emit Unpaused(msg.sender, 1);
        }
    }
    ...
    ) external payable nonReentrant whenRelayNotPaused {
