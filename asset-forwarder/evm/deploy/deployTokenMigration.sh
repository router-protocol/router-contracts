...
OLD_TOKEN="0xbB60d1ca0C5121f46243c28D935795C43795d1D1"
NEW_TOKEN="0x5226d56F25DCDE6199791FC8793bea6803883bEb"
MULTIPLIER=333300
...
if [ -z "$VERIFY" ]; then
  ...
  forge create --rpc-url ${rpc} --private-key ${privateKey} contracts/TokenMigration.sol:TokenMigration \
    --constructor-args  "${OLD_TOKEN}" "${NEW_TOKEN}" "${MULTIPLIER}" --etherscan-api-key ${etherscanApiKey} --verify
else
  echo "Running Deployment Command without Verification"
  forge create --rpc-url ${rpc} --private-key ${privateKey} contracts/TokenMigration.sol:TokenMigration \
    --constructor-args  "${OLD_TOKEN}" "${NEW_TOKEN}" "${MULTIPLIER}" --legacy
fi
