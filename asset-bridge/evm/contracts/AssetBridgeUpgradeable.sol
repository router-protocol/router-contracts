    ...
    IAssetBridge
{
    ...
    function setRequestMetadata(bytes memory requestMetadata) external payable onlyRole(RESOURCE_SETTER) {
        AssetBridge_REQUEST_METADATA = requestMetadata;
    }
    ...
    function safeTransferFrom(address token, address from, address to, uint256 amount) internal {
        if (from == address(this)) {
            // TRON USDT
            if (token == address(0xa614f803B6FD780986A42c78Ec9c7f77e6DeD13C)) {
